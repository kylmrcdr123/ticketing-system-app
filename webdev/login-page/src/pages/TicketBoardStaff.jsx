...
const TicketTableAdmin = () => {
  ...
  const handleStatusChange = async (ticketId, newStatus) => {
    ...
    try {
      const response = await fetch(`https://7b9b-209-35-171-143.ngrok-free.app/TicketService/updateStatus/${ticketId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
                  ...
                  {filteredTickets[status].length > 0 ? (
                    filteredTickets[status].map((ticket) => (
                      <div key={ticket?.ticketId?.toString() || 'default-id'} className="ticket-item">
                        ...
                        {/* Only render the status dropdown if the ticket is not closed */}
